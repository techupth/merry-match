...
filterRouter.post("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `select * from users where (user_age between $1 and $2) and (meeting_int = $3 or meeting_int = $4 or meeting_int = $5 or meeting_int = $6 or meeting_int = $7) and (sex_pref = $8) and (user_id not like $9) `,
      [
        filter.ageRange[0],
        filter.ageRange[1],
        filter.meetingInt[0],
